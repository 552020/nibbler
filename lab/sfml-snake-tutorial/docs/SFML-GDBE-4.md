...
> **Note**: The `sf::Event` enumeration values can be different, depending on which version of SFML you are using!
...
The constructor's job in this case is really simple. All it has to do is call a private method `LoadBindings()`, which is used to load the information about our bindings from a file. We will cover that shortly. The destructor's job is also fairly run-of-the-mill for this type of class. If you recall, we store the bindings on the heap, so this dynamic memory has to be de-allocated. Let's take a gander at the `AddBinding` method implementation:
