...
// , m_world(sf::Vector2u(800, 600))        // Will be in State_Game
// , m_snake(m_world.GetBlockSize())        // Will be in State_Game
// , m_sprite(m_texture)                    // Will be in State_Mushroom
, m_stateManager(&m_context)
{
    m_clock.restart();
    
    // NOTE: srand(time(nullptr)) should be in State_Game::OnCreate() instead
    // because World (which uses rand() for apple placement) will be initialized there.
    // This is more appropriate than seeding in Game constructor.
    
    // Initialize shared context
    m_context.m_wind = &m_window;
    m_context.m_eventManager = m_window.GetEventManager();
    
    // Start with game state for debugging
    m_stateManager.SwitchTo(StateType::Game);
    
    // COMMENTED OUT: Textbox setup - may be used in State_Game or removed
    // m_textbox.Setup(5, 14, 350, sf::Vector2f(225, 0));
    // m_textbox.Add("Seeded random number generator with: " + std::to_string(time(NULL)));
    
    // COMMENTED OUT: Texture and sprite setup - will be in State_Mushroom
    ...
    // COMMENTED OUT: Global callback for MoveSprite - will be in State_Mushroom
...
// COMMENTED OUT: Snake input handling - will be moved to State_Game using EventManager
...
// COMMENTED OUT: MoveSprite callback - will be in State_Mushroom
